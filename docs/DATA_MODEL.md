...
| `title` | `VARCHAR(200)` | Yes | — | Trimmed, non-blank; use an intentional title such as `Untitled`, not null |
...
## 3. Proposed Initial Migration

The authoritative DDL will be implemented as `V1__create_photos.sql` in Step 4. The design target is:
...
Application validation remains stricter where needed (`@NotBlank`, length, `@PastOrPresent`, upload limit). Database constraints provide a final integrity boundary and protect writes outside normal controllers.
...
- `title`: caption and accessible alt text need a deliberate value
...
5. Setting visibility to PUBLIC does not require EXIF/location/description, but still requires title, capture date, and a valid asset.
...
Contains the same mutable metadata as create except `cloudinaryPublicId`. `title`, `takenAt`, `featured`, and `visibility` are required in every PUT; optional fields may be explicitly `null` to clear them.
...
An `AdminPhotoResponse` may include visibility, verified dimensions, update timestamps, and the internal public ID needed for operational management. It is never reused as a public DTO and every endpoint returning it requires the admin session.
      ...
      "months": [
        {
          ...
          "photoCount": 2,
          "photos": [
            { "id": "...", "title": "...", "takenAt": "2026-08-14", "image": {} }
          ]
        },
        {
          "month": 7,
